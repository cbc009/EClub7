...
- (void)viewDidLoad
{
    ...
    UIImageView *bg_View =[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, DeviceFrame.size.width, DeviceFrame.size.height)];
    [bg_View setImage:FH_GetWithImage(@"ScratchCard", @"png")];
    
    [bg_View setContentMode:UIViewContentModeScaleToFill];
    
    UIView *view =[[UIView alloc] initWithFrame:CGRectMake(30, DeviceFrame.size.height/2+100, DeviceFrame.size.width-60, 60)];
  ...
  [[self view] addSubview:bg_View];
    ...
    [prizeService prizePrizeLuckiesWithToken:user.token andUser_type:user.user_type inTabBarController:self.tabBarController WithDone:^(PrizeLuckiesInfo *model){
        ...
        [scratchView setFrame:CGRectMake(30, DeviceFrame.size.height/2+100, DeviceFrame.size.width-60, 60)];
